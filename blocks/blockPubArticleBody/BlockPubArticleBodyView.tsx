...
const BlockPubArticleBodyView: FC<BlockViewProps<BlockPubArticleBodyProps>> = props => {
  useEffect(() => {
    try {
  ...
  if (articleReferenceBlockData?.tags) {
    tagListBlock = articleReferenceBlockData?.tags.reduce((tot: TagData[], tagId: string): TagData[] => {
      const tagBlock = articleReferenceBlock.listExternalBlocks?.items.find(externalBlock => externalBlock.id === tagId)
      const blockTagData = tagBlock?.data ? (JSON.parse(tagBlock.data) as BlockPubSectionMainViewProps) : null
      ...
      <div className="progress-bar-section">
        <div className="main-container">
          <div className="article-progress-bar-title">
            <h6 className="no-bottom-margin">{articleReferenceBlockData.title}</h6>
          </div>
